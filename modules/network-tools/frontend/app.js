(function () {
  ...
  function stageLabel(id){var labels={default_route:[`\u041c\u0430\u0440\u0448\u0440\u0443\u0442 \u043f\u043e \u0443\u043c\u043e\u043b\u0447\u0430\u043d\u0438\u044e`,'Default route'],interface:[`\u0421\u0435\u0442\u0435\u0432\u043e\u0439 \u0438\u043d\u0442\u0435\u0440\u0444\u0435\u0439\u0441`,'Network interface'],local_address:[`\u041b\u043e\u043a\u0430\u043b\u044c\u043d\u044b\u0439 \u0430\u0434\u0440\u0435\u0441`,'Local address'],gateway:[`\u0428\u043b\u044e\u0437`,'Gateway'],internet:[`\u0414\u043e\u0441\u0442\u0443\u043f \u0432 \u0418\u043d\u0442\u0435\u0440\u043d\u0435\u0442`,'Internet connectivity'],dns:['DNS','DNS'],target_route:[`\u041c\u0430\u0440\u0448\u0440\u0443\u0442 \u043a \u0446\u0435\u043b\u0438`,'Target route'],policy_routing:['Политика маршрутизации','Policy routing'],kernel_egress:['Выходной интерфейс ядра','Kernel-selected egress'],gateway_consistency:['Согласованность шлюза','Gateway/interface consistency'],source_consistency:['Исходный адрес маршрута','Route/source consistency'],ping:['Ping','Ping'],tcp:['TCP','TCP'],http:['HTTP','HTTP']};var pair=labels[id]||[id,id];return lx(pair[0],pair[1]);}
  ...
  function verdictCopy(v){var code=(v&&v.code)||'healthy';var map={healthy:[`\u0421\u0435\u0442\u0435\u0432\u043e\u0439 \u043f\u0443\u0442\u044c \u0438\u0441\u043f\u0440\u0430\u0432\u0435\u043d`,'Network path is healthy'],degraded:[`\u0421\u0435\u0442\u044c \u0440\u0430\u0431\u043e\u0442\u0430\u0435\u0442 \u0441 \u043e\u0433\u0440\u0430\u043d\u0438\u0447\u0435\u043d\u0438\u044f\u043c\u0438`,'Network is operating with warnings'],no_default_route:[`\u041d\u0435\u0442 \u043c\u0430\u0440\u0448\u0440\u0443\u0442\u0430 \u043f\u043e \u0443\u043c\u043e\u043b\u0447\u0430\u043d\u0438\u044e`,'No default route'],interface_down:[`\u0412\u044b\u0445\u043e\u0434\u043d\u043e\u0439 \u0438\u043d\u0442\u0435\u0440\u0444\u0435\u0439\u0441 \u043d\u0435 \u0433\u043e\u0442\u043e\u0432`,'Default interface is down'],no_local_address:[`\u041d\u0435\u0442 \u0440\u0430\u0431\u043e\u0447\u0435\u0433\u043e IP-\u0430\u0434\u0440\u0435\u0441\u0430`,'No usable local address'],internet_unreachable:[`\u0412\u043d\u0435\u0448\u043d\u044f\u044f \u0441\u0435\u0442\u044c \u043d\u0435\u0434\u043e\u0441\u0442\u0443\u043f\u043d\u0430`,'Internet connectivity failed'],dns_failure:[`DNS \u043d\u0435 \u0440\u0430\u0431\u043e\u0442\u0430\u0435\u0442`,'DNS resolution failed'],target_route_failure:[`\u041d\u0435\u0442 \u043c\u0430\u0440\u0448\u0440\u0443\u0442\u0430 \u043a \u0446\u0435\u043b\u0438`,'No route to target'],egress_interface_failure:['Выбранный ядром интерфейс недоступен','Kernel-selected egress is unavailable'],gateway_interface_mismatch:['Шлюз и выходной интерфейс маршрута не согласованы','Route gateway/interface mismatch'],route_source_mismatch:['Исходный адрес не принадлежит выбранному интерфейсу','Route source does not belong to selected interface'],target_unreachable:[`\u0426\u0435\u043b\u044c \u043d\u0435 \u043e\u0442\u0432\u0435\u0447\u0430\u0435\u0442 \u043d\u0430 Ping`,'Target does not answer Ping'],tcp_failure:[`TCP-\u0441\u0435\u0440\u0432\u0438\u0441 \u043d\u0435\u0434\u043e\u0441\u0442\u0443\u043f\u0435\u043d`,'TCP service is unreachable'],http_failure:[`HTTP-\u043f\u0440\u043e\u0432\u0435\u0440\u043a\u0430 \u043d\u0435 \u043f\u0440\u043e\u0448\u043b\u0430`,'HTTP check failed']};var pair=map[code]||[code,code];return lx(pair[0],pair[1]);}
  function actionLabel(id){var map={'review-warnings':['Проверить предупреждения','Review warnings'],'inspect-default-route':['Проверить default route','Inspect default route'],'inspect-policy-rules':['Проверить PBR / policy rules','Inspect PBR / policy rules'],'inspect-default-interface':['Проверить выходной интерфейс','Inspect default interface'],'inspect-interface-address':['Проверить адрес интерфейса','Inspect interface address'],'inspect-kernel-egress':['Проверить kernel-selected egress','Inspect kernel-selected egress'],'inspect-target-route':['Проверить маршрут к цели','Inspect target route'],'inspect-source-address':['Проверить исходный адрес','Inspect source address'],'verify-upstream':['Проверить upstream','Verify upstream'],'inspect-dns':['Проверить DNS','Inspect DNS'],'verify-service':['Проверить сервис','Verify service'],'verify-target-reachability':['Проверить доступность цели','Verify target reachability'],'review-diagnosis':['Проверить диагностику','Review diagnosis']};var pair=map[id]||[id,id];return lx(pair[0],pair[1]);}
  function renderDoctorActions(actions){if(!actions||!actions.length)return '';var rows=actions.map(function(a){var tone=a.priority==='high'?'bad':a.priority==='medium'?'warn':'';return '<div class="nt-result-row"><span class="nt-result-icon">›</span><strong>'+U.esc(actionLabel(a.id))+'</strong><span class="mono">'+U.esc(a.fault_domain||'')+'</span>'+U.badge(String(a.priority||'').toUpperCase(),tone)+'<span class="nt-result-meta">'+U.esc(a.detail||'')+'</span></div>';}).join('');return '<div class="doctor-selected-title">'+U.esc(lx('Что проверить дальше','Next checks'))+'</div>'+rows;}
