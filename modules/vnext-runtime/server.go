...
func serveModule(cfg moduleConfig) error {
	...
	mux.HandleFunc("/v1/health", getOnly(func(w http.ResponseWriter, _ *http.Request) {
		writeJSON(w, http.StatusOK, map[string]any{
			"ok": true, "module": cfg.ID, "version": version, "api_version": 1,
			"mode": "read-only-foundation", "mutation_api": false,
			"uptime_seconds": time.Since(started).Seconds(),
		})
	}))

	registerModuleRoutes(mux, cfg)
...
func registerUIRoutes(mux *http.ServeMux, uiPath string) {
	...
	mux.HandleFunc("/v1/ui/", func(w http.ResponseWriter, r *http.Request) {
		...
		if clean == "." || clean == "index.html" {
			...
			if err != nil {
				http.Error(w, "module UI unavailable", http.StatusServiceUnavailable)
				return
			}
		...
		if strings.Contains(r.URL.Path, "/_app/immutable/") {
...
func registerModuleRoutes(mux *http.ServeMux, cfg moduleConfig) {
	switch cfg.ID {
	case "maintenance":
	...
	case "integrations":
		registerIntegrationsRoutes(mux)
	case "developer-tools":
		registerDeveloperToolsRoutes(mux)
	}
}
