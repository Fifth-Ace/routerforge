...
<style>
  .editor-backdrop{position:fixed;inset:0;z-index:998;background:rgba(0,0,0,.58);backdrop-filter:blur(1.5px);animation:editor-backdrop-in .30s ease both}
  .editor-backdrop.closing{animation:editor-backdrop-out .24s ease both}
  .editor-sheet{position:fixed;z-index:999;top:0;left:0;right:0;height:75vh;display:grid;grid-template-rows:auto auto 1fr auto;background:var(--rf-bg,#0b0d10);color:var(--rf-text,#f5f7fa);border-bottom:1px solid var(--rf-border,#29313a);box-shadow:0 24px 65px rgba(0,0,0,.46);font-family:Inter,system-ui,sans-serif;animation:sheet-in .32s cubic-bezier(.22,.72,.2,1) both;will-change:transform,opacity}
  ...
  .editor-topbar{display:flex;justify-content:space-between;gap:1rem;align-items:center;padding:.85rem 1rem;background:var(--rf-surface,#12151a);border-bottom:1px solid var(--rf-border,#29313a)}
  .editor-title{min-width:0;display:flex;flex-direction:column;gap:.16rem}
  .editor-title strong{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-family:"Roboto Mono","Cascadia Mono",Consolas,monospace;font-size:.9rem;color:var(--rf-text,#f5f7fa)}
  .editor-kicker{display:flex;align-items:center;gap:.42rem;color:var(--rf-accent,#38bdf8);font-size:.66rem;font-weight:800;letter-spacing:.065em;text-transform:uppercase}
  .editor-kicker em{border:1px solid var(--rf-accent-border,rgba(56,189,248,.30));border-radius:var(--rf-radius-control,.32rem);padding:.08rem .32rem;color:var(--rf-accent,#38bdf8);background:var(--rf-accent-soft,rgba(56,189,248,.10));font-size:.56rem;font-style:normal}
  .editor-dot{width:.46rem;height:.46rem;border-radius:50%;background:var(--good,#2ea043);box-shadow:0 0 0 3px color-mix(in srgb,var(--good,#2ea043) 14%,transparent)}
  .editor-path{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--rf-muted,#8d98a4);font-family:"Roboto Mono","Cascadia Mono",Consolas,monospace;font-size:.65rem}
  .editor-actions{display:flex;align-items:center;gap:.42rem;flex:none}
  button{border:1px solid var(--rf-border-strong,#36414d);border-radius:var(--rf-radius-control,.4rem);background:var(--rf-surface-2,#171b21);color:color-mix(in srgb,var(--rf-text,#f5f7fa) 86%,var(--rf-muted,#8d98a4));padding:.38rem .58rem;font:inherit;font-size:.7rem;cursor:pointer}
  button:hover:not(:disabled){background:var(--rf-hover,#1d2229);border-color:var(--rf-accent-border,rgba(56,189,248,.30))}
  button:disabled{opacity:.4;cursor:not-allowed}
  button.close{width:2rem;height:2rem;padding:0;font-size:1.15rem;line-height:1;border-color:rgba(248,81,73,.38);color:var(--bad,#f85149)}
  .editor-state{border:1px solid color-mix(in srgb,var(--good,#2ea043) 38%,transparent);border-radius:var(--rf-radius-control,.34rem);color:var(--good,#2ea043);background:color-mix(in srgb,var(--good,#2ea043) 9%,transparent);padding:.18rem .38rem;font:700 .58rem/1 "Roboto Mono","Cascadia Mono",Consolas,monospace;letter-spacing:.04em}
  .editor-state.dirty{color:var(--warn,#d29922);border-color:color-mix(in srgb,var(--warn,#d29922) 42%,transparent);background:color-mix(in srgb,var(--warn,#d29922) 9%,transparent)}
  .editor-state.readonly{color:var(--rf-accent,#38bdf8);border-color:var(--rf-accent-border,rgba(56,189,248,.30));background:var(--rf-accent-soft,rgba(56,189,248,.10))}
  .editor-meta,.editor-footer{min-height:1.85rem;display:flex;align-items:center;gap:.85rem;padding:0 .85rem;background:var(--rf-surface-2,#171b21);color:var(--rf-muted,#8d98a4);border-bottom:1px solid var(--rf-border,#29313a);font:.61rem/1 "Roboto Mono","Cascadia Mono",Consolas,monospace}
  .editor-footer{border-top:1px solid var(--rf-border,#29313a);border-bottom:0;min-height:2.2rem}
  .editor-footer button{padding:.28rem .5rem}.spacer{flex:1}
  .editor-surface{min-height:0;display:grid;grid-template-columns:auto 1fr;background:var(--rf-bg,#0b0d10);overflow:hidden}
  .editor-gutter{box-sizing:border-box;margin:0;min-width:3.8rem;height:100%;overflow:hidden;padding:.9rem .72rem .9rem .4rem;border-right:1px solid var(--rf-border,#29313a);background:var(--rf-surface,#12151a);color:color-mix(in srgb,var(--rf-muted,#8d98a4) 74%,var(--rf-bg,#0b0d10));text-align:right;user-select:none;white-space:pre;font:12.5px/1.55 "Roboto Mono","Cascadia Mono",Consolas,monospace}
  .editor-code-wrap{position:relative;min-width:0;min-height:0;overflow:hidden;background:var(--rf-bg,#0b0d10)}
  .editor-highlight,.editor-text{box-sizing:border-box;position:absolute;inset:0;width:100%;height:100%;margin:0;padding:.9rem 1rem;overflow:auto;white-space:pre;tab-size:2;font:12.5px/1.55 "Roboto Mono","Cascadia Mono",Consolas,monospace}
  .editor-highlight{z-index:1;border:0;background:var(--rf-bg,#0b0d10);color:color-mix(in srgb,var(--rf-text,#f5f7fa) 88%,var(--rf-muted,#8d98a4));pointer-events:none;scrollbar-width:none}
  .editor-highlight::-webkit-scrollbar{display:none}
  .editor-text{z-index:2;resize:none;border:0;outline:0;background:transparent;color:transparent;caret-color:var(--rf-text,#f5f7fa);-webkit-text-fill-color:transparent}
  .editor-text::selection{background:var(--rf-accent-hover,rgba(56,189,248,.16));-webkit-text-fill-color:transparent}
  .editor-text:focus{box-shadow:inset 2px 0 0 var(--rf-accent,#38bdf8)}
  .editor-text:read-only{caret-color:var(--rf-muted,#8d98a4)}
  :global(.tok-comment){color:#5c7080;font-style:italic}:global(.tok-string){color:#ffd866}:global(.tok-key){color:#8bc8ff}:global(.tok-number){color:#c099ff}:global(.tok-boolean),:global(.tok-null){color:#ff9da4}:global(.tok-keyword){color:#ff7ab2;font-weight:650}:global(.tok-function){color:#82d2ce}:global(.tok-variable){color:#7bd88f}:global(.tok-operator){color:#89ddff}:global(.tok-section){color:#c099ff;font-weight:700}:global(.tok-tag){color:#ff7ab2}:global(.tok-selector){color:#7bd88f}:global(.tok-link){color:var(--rf-accent,#38bdf8);text-decoration:underline}:global(.tok-address){color:#ffb86c}:global(.tok-path){color:#82d2ce}
  ...
  @media(prefers-reduced-motion:reduce){.editor-sheet,.editor-sheet.closing,.editor-backdrop,.editor-backdrop.closing{animation-duration:.01ms}}
</style>
