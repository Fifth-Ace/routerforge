...
### Производительность
- RouterForge делает заметно меньше лишней работы в фоне, когда интерфейс закрыт или отдельные разделы не используются.
- Monitoring собирает тяжёлые данные только по запросу и в простое почти не использует процессор.
- DNS экономнее расходует память и реже запускает фоновые проверки; лимиты истории автоматически подстраиваются под объём памяти роутера.
- Системные проверки диска, температуры и служб выполняются реже, без потери основной функциональности.

### Проверка на роутере
- Изменения проверены на Keenetic Hopper KN-3811.
- В простое четыре основных процесса RouterForge вместе использовали в среднем меньше 1% CPU.
- В сопоставимом замере Core, DNS и Monitoring вместе использовали примерно на 20% меньше памяти, чем до оптимизации.

### Release
- Beta train обновлён до `0.10.0-beta.4` (`0.10.0~beta.4` для opkg).
- Состав Beta остаётся прежним: 7 компонентов × 3 архитектуры = 21 IPK.
- Immutable snapshot: `routerforge-v0.10.0-beta.4`.
- Stable/main этим выпуском не обновляется.
