<script>
  ...
  function editorMode(name) {
    ...
    if (lower.endsWith('.js') || lower.endsWith('.mjs') || lower.endsWith('.ts')) return 'JS';
    if (lower.endsWith('.css')) return 'CSS';
    if (lower.endsWith('.html') || lower.endsWith('.htm')) return 'HTML';
    if (lower.endsWith('.go')) return 'GO';
    if (lower.endsWith('.sh')) return 'SHELL';
    if (lower.endsWith('.conf') || lower.endsWith('.ini') || lower.endsWith('.env')) return 'CONF';
    return 'TEXT';
  }
  ...
  function highlightLine(line, mode) {
    ...
    if (mode === 'CONF') {
      ...
      if (kv) return `${escapeHtml(kv[1])}${token(kv[2], 'key')}${token(kv[3], 'operator')}${highlightLine(kv[4], 'CONF_VALUE')}`;
    }
    if (mode === 'CONF_VALUE') {
      if (/^(true|false|yes|no|on|off)$/i.test(line.trim())) return token(line, 'boolean');
    }
    if (mode === 'HTML') {
      let out = ''; let i = 0;
      while (i < line.length) {
    ...
    while (i < line.length) {
      ...
      if ((mode === 'SHELL' || mode === 'CONF_VALUE') && ch === '#') { out += token(line.slice(i), 'comment'); break; }
      if ((mode === 'JS' || mode === 'GO' || mode === 'CSS') && ch === '/' && next === '/') { out += token(line.slice(i), 'comment'); break; }
      ...
      if (ch === '"' || ch === "'" || ((mode === 'JS' || mode === 'GO' || mode === 'SHELL') && ch === '`')) {
      ...
      if (mode === 'SHELL' && ch === '$') {
        ...
        out += token(line.slice(i, j), 'variable'); i = j; continue;
      }
      if (isIdentStart(ch)) {
        ...
        else if (keywords[mode]?.has(word)) out += token(word, ['true','false','null','nil','undefined'].includes(word) ? 'boolean' : 'keyword');
        else if ((mode === 'JS' || mode === 'GO') && /^\s*\(/.test(line.slice(j))) out += token(word, 'function');
