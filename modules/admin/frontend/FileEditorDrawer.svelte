<script>
  ...
  $: copy = locale === 'ru' ? {
    ...
    saved: 'SYNCED',
    readOnly: 'READ ONLY',
    discard: 'Close editor and discard unsaved changes?',
...
<aside class="editor-drawer" aria-label={copy.editor}>
  <header class="editor-topbar">
    ...
    <div class="editor-actions">
      <span class:dirty class:readonly={readOnly} class="editor-state">
        {readOnly ? copy.readOnly : (dirty ? copy.dirty : copy.saved)}
      </span>
      <button type="button" onclick={onSave} disabled={!dirty || busy || readOnly}>
        {busy ? '…' : copy.save}
      </button>
  ...
  <div class="editor-surface">
    ...
    <textarea
      ...
      aria-label={file?.path || copy.editor}
    ></textarea>
  </div>
  ...
  <footer class="editor-footer">
    <span>{copy.hint}</span>
    <span class="spacer"></span>
    <button type="button" onclick={requestClose}>{copy.close}</button>
  </footer>
</aside>
...
<style>
  ...
  button:hover:not(:disabled){background:#202d3a;border-color:#4b657d}
  button:disabled{opacity:.4;cursor:not-allowed}
  button.close{
    width:2rem;height:2rem;padding:0;
    font-size:1.15rem;line-height:1;
    border-color:#583941;color:#ff9da4;
  }
  .editor-state{
  ...
  .editor-state.dirty{color:#ffd866;border-color:#5b512d}
  .editor-state.readonly{color:#c099ff;border-color:#59446f}
  .editor-meta,.editor-footer{
    min-height:1.85rem;
    display:flex;
    align-items:center;
    gap:.85rem;
  ...
  .editor-footer button{padding:.28rem .5rem}
  .spacer{flex:1}
  .editor-surface{
    min-height:0;
    display:grid;
  ...
  .editor-text::selection{background:#29445f;color:#fff}
  .editor-text:focus{box-shadow:inset 2px 0 0 #5de4c7}
  .editor-text:read-only{color:#94a0ac}
  @media(max-width:760px){
    .editor-drawer{width:96vw}
    .editor-meta span:nth-child(3),.editor-meta span:nth-child(4){display:none}
    .editor-footer>span:first-child{display:none}
  }
  @media(prefers-reduced-motion:reduce){
    .editor-drawer{animation:none}
  }
</style>
