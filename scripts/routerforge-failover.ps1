...
function Invoke-GitNative([string[]]$A){$o=$ErrorActionPreference;$ErrorActionPreference='Continue';$r=& git @A 2>&1;$c=$LASTEXITCODE;$ErrorActionPreference=$o;if($c-ne0){Fail(('git '+($A-join' ')+' rc='+$c+' '+($r-join' | ')))};@($r)}
...
function SetCfg([string]$K,[string]$V){if([string]::IsNullOrWhiteSpace($V)){$o=$ErrorActionPreference;$ErrorActionPreference='Continue';& git config --unset-all $K 2>$null;$c=$LASTEXITCODE;$ErrorActionPreference=$o;if($c-ne0-and$c-ne5){Fail('unset '+$K+' rc='+$c)}}else{Invoke-GitNative @('config',$K,$V)|Out-Null}}
...
function AssertRepo(){if(!(Test-Path '.git')){Fail 'run from RouterForge repo root'};$o=([string](Invoke-GitNative @('remote','get-url','origin'))).Trim();$f=([string](Invoke-GitNative @('remote','get-url','forgejo'))).Trim();if($o-ne$GitHub){Fail('origin mismatch: '+$o)};if($f-ne$Forgejo){Fail('forgejo mismatch: '+$f)};if(@(Invoke-GitNative @('status','--porcelain')).Count-ne0){Fail 'worktree/index must be clean'}}
function Status(){
...
function Promote(){
 ...
 try{Invoke-GitNative @('config','remote.pushDefault','forgejo')|Out-Null;Invoke-GitNative @('config','branch.dev.remote','forgejo')|Out-Null;Invoke-GitNative @('config','branch.dev.merge','refs/heads/dev')|Out-Null;Invoke-GitNative @('config','branch.main.remote','forgejo')|Out-Null;Invoke-GitNative @('config','branch.main.merge','refs/heads/main')|Out-Null}catch{SetCfg 'remote.pushDefault' ([string]$s.previous_push_default);SetCfg 'branch.dev.remote' ([string]$s.previous_dev_remote);SetCfg 'branch.main.remote' ([string]$s.previous_main_remote);Remove-Item $StateFile -Force -ErrorAction SilentlyContinue;throw}
...
function Restore(){
 ...
 SetCfg 'remote.pushDefault' ([string]$s.previous_push_default);SetCfg 'branch.dev.remote' ([string]$s.previous_dev_remote);SetCfg 'branch.main.remote' ([string]$s.previous_main_remote);Invoke-GitNative @('config','branch.dev.merge','refs/heads/dev')|Out-Null;Invoke-GitNative @('config','branch.main.merge','refs/heads/main')|Out-Null;Remove-Item $StateFile -Force
